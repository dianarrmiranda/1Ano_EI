...
diferencaGolos = lambda reg: reg[GM]-reg[GS]
...
def main():
    ...
    tab = sorted(tabela, key = diferencaGolos, reverse=True)
    printTabela(tab)
    ...
    tab = sorted(tabela, reverse=True, key= lambda reg: (pontos(reg), diferencaGolos(reg)))
    printTabela(tab)
